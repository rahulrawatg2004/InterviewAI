...
export const generateQuestion = async (req, res) => {
  try {
    ...
    `;
    if (!userPrompt.trim()) {
      return res.status(400).json({ message: "Prompt content is heavy" });
    }
    const messages = [
      {
        role: "system",
      ...
      Make questions based on the candidates'role , experience , project 
      skills,interview Mode and resume details
              `,
      },
      {
        role: "user",
    ...
    const messages = [
      {
        role: "system",
        content: `
You are a professional human interviewer evaluating a candidate's answer in
real interview.
